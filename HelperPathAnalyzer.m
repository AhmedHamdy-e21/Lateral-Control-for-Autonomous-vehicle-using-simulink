classdef HelperPathAnalyzer < matlab.System & matlab.system.mixin.Propagates & ...
    ...
    %     HelperCubicSplineFit, HelperSpeedProfileGenerator
    ...
    properties(Access = private)
        ...
        % The following three properties are used to transfer reference 
        % data within the system object. Depending on the environment the 
        % object is executing in, they are assigned either by public
        % properties, RefPoses, SpeedProfile and Directions in MATLAB, or
        % by the input ports in Simulink. The selection is determined by
        % the HasReferenceInports property.
    ...
    methods(Access = protected)
        ...
        function setupImpl(obj, ~, ~, varargin)
            ...
            if isSimulinkBlock(obj) % In Simulink
                ...
                obj.SpeedProfileInternal = nan(size(varargin{3}), 'like', varargin{3});
            else % In MATLAB
        ...
        %------------------------------------------------------------------
        function [refPose, refVel, direction, varargout] = stepImpl(obj, ...
            ...
            if obj.HasReferenceInports 
                ...
                if ~isequal(obj.RefPosesInternal, varargin{1})
                    ...
                    obj.SpeedProfileInternal = varargin{3};
            ...
            [refPose, refVel, direction] = findDesiredPoseAndVelocity(obj, currPose);
        ...
        %------------------------------------------------------------------
        function [refPose, refVel, direction] = findDesiredPoseAndVelocity(obj, pose)
    ...
    methods(Access = protected)
        ...
        function flag = isInputSizeMutableImpl(~,index)
            ...
            % refPoses, directions and speeProfile are variable-size inputs
            if any(index == [3,4,5])
                flag = true;
            else
                flag = false;
